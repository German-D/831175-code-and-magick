...
var randomNumber = function (maxNumber) {
  var result = Math.round(Math.random() * maxNumber);
  return result;
};

for (var i = 0; i < manyMages.length; i++) {
  manyMages[i].name = manyNames[randomNumber(7)];
  manyMages[i].surname = manySurnames[randomNumber(7)];
  manyMages[i].coatColor = manyCoatColor[randomNumber(5)];
  manyMages[i].eyesColor = manyEyesColor[randomNumber(4)];
}
