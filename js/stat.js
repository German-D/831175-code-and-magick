...
for (var i = 0; i < names.length; i++) {
   ...
   if (names[i] === 'Вы') {
    ...
    ctx.fillStyle = 'rgba(0, 153, 255, parseFloat(Math.random().toFixed(1))';
   };
    ctx.fillText(names[i], cloudX + gap + (gap + barWidth)*i, nameHeight);
    ctx.fillRect(cloudX + gap + (gap + barWidth)*i, cloudHeight - barHeight - fontGap, barWidth, (barHeight * times[i]) / maxTime);
  }
